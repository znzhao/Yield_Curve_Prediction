...
from helper.sensitive_utils import fred_api_key, pushToGithub
...
def parseYieldCurve(value = 'daily_treasury_yield_curve', start = '199001', git = False, *args, **kwargs):
    '''
...
def parseFredData(series_id, observation_start = '2003-01-02', observation_end = '9999-12-31', *args, **kwargs):
    ...
    '''
    # download data from the US treasury
    fred_html = 'https://api.stlouisfed.org/fred/series/observations?'
    fred_html += 'series_id={series_id}'.format(series_id = series_id)
    fred_html += '&api_key={api_key}'.format(api_key = fred_api_key)
    fred_html += '&observation_start={observation_start}'.format(observation_start = observation_start)
    fred_html += '&observation_end={observation_end}'.format(observation_end = observation_end)
...
def parseBreakEvenInflation(git = False):
    '''
    ...
    if git:
        pushToGithub('MktData/tyie/tyie.csv', tyie.to_csv(index=True, header=True))
    else:
...
def parseYahooData(ticker, git):
    ...
    if git:
        pushToGithub('MktData/stock/{}.csv'.format(name), data.to_csv(index=True, header=True))
    else:
...
def parseSP500(git = False):
    sp500 = parseYahooData('SP500', git)
    return sp500
